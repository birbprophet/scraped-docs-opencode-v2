---
url: https://opencode.ai/v2/docs/tools
title: "Tools"
description: "Tools documentation for OpenCode."
...
data. Other binary files fail. See [Attachments](attachments.md) for supported
...
`external_directory` approval. See [Permissions](permissions.md) for path
normalization, rule order, and saved approvals. To make outside material
available by name, configure [References](references.md).
...
authority, so use narrow rules in [Permissions](permissions.md).
...
Permission: `websearch` with the query as its resource. See [Websearch](websearch.md)
...
Permission: `skill` with the skill ID as its resource. See [Skills](skills.md) for
...
[Skills](skills.md) add task instructions rather than new executable tools.
[Attachments](attachments.md) put user-selected content into a prompt, while
[References](references.md) provide named outside directories. These features
